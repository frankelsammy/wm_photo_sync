...
def get_date(url):
    # Split on 'AWTI_VIN_' and then '_'
    parts = url.split("AWTI_VIN_")[1].split("_")
    ...
    with sync_playwright() as p:
        ...
        for customer_id in customer_id_map.keys():
            ...
            for idx, url in enumerate(media_urls, start=1):
                if get_date(url) < date.today():
                    print(f"Skipping media {idx} as it is older than today: {url}")
                    continue
